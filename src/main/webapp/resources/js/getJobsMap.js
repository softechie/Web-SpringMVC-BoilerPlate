...
class JobsMap extends React.Component {
    ...
    _drawGrid() {
        ...
        var counter = 0;
        for (var x = .5; x < width; x += this.state.scale) {
            context.moveTo(x, 0);
            if (counter < 10)
        ...
        counter = 0;
        for (var y = .5; y < height; y += this.state.scale) {
            context.moveTo(0, y);
        ...
        context.fillText("Jobs Map", context.canvas.width / 2, 50);
    ...
    _drawNode(job, xPos, yPos) {
        ...
        context.fillText(jobName, xRounded + (this.state.scale / 10), yRounded + (this.state.scale / 10));
        context.stroke();
    ...
    //checks the whole list of jobs to get the information on the dependency
    //jobs, then returns them as an object array, and also checks if the job
    //has been already drawn, and will not include it if so
    ...
    _drawLine(startX, startY, endX, endY) {
        ...
        context.lineTo(edgeEndX-headlen*Math.cos(angle-Math.PI/6), edgeEndY-headlen*Math.sin(angle-Math.PI/6));
                ...
                {this.state.nodes.map(function(node) {
                    ...
                    return (
                        <div id={nodeId+"-content"} className="dropdown-content" key={nodeId+"-content"}>
                            <div>Job Category: {node.category}</div>
                            <div>Job Name: {node.name}</div>
                            <div>Reference Name: {node.ref}</div>
                            <div>Job Status: {node.status}</div>
                            <div>Schedule Status: {node.scheduled}</div>
                            <div>Job Dependencies:</div>
